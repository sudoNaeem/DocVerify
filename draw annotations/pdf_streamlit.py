...
class PDFAnnotator:
    ...
    def save_pdf(self, pdf_name):
        if self.pdf_document:
            self.pdf_document.save(pdf_name)
            with open('annotations.json', 'w') as f:
                json.dump(self.boxes, f, indent=4)
            return f"PDF saved as: {pdf_name}. Annotations saved in annotations.json."
...
if uploaded_file is not None:
    ...
    save_filename = st.text_input("Save as", value="annotated.pdf")
    if st.button("Save PDF"):
        result = pdf_annotator.save_pdf(save_filename)
        st.write(result)
        ...
        # Create a link to download the annotations JSON
        with open("annotations.json", "rb") as file:
            st.download_button(
